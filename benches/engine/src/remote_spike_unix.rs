//! Unix-only remote-transfer spike. Compiled in from the
//! `xsync-remote-spike` binary shim, which stubs itself out elsewhere.
//!
//! Lives outside `src/bin/` so cargo does not autodiscover it as a second
//! binary target with no `main`.
