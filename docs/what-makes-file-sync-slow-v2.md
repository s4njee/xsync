...
the measurement log, the three false conclusions we published before catching
them, and the two serialisation bugs that were hiding inside what we had
labelled "the OS".*
...
Note the shapes. congress and cb7 have nearly identical directory density — we
...
was failing, and the failure path took a stable ~7.85 s. Worse, we then *explained*
...
We then varied the *sender* deliberately — same corpora, same receivers, three
...
also our leading suspect for a 5.3 ms in-session SSH round trip — absurd for a
...
**3.32×** after. A third of what we had attributed to Windows was xsync
...
serialised tool is partly a measurement of the tool**, and we published the larger
number first.
...
we measured *that* Windows charges a large fixed cost per file creation. We did
...
Candidates, in rough order of our confidence: MFT record allocation and the
...
NTFS vs ReFS on the same box, via a Dev Drive. **We deliberately did not run
it**: Dev Drives get different antivirus treatment by default, which
reintroduces exactly the confound we spent a day eliminating, and no engineering
...
sub-millisecond wired. "Not CPU-bound" is a claim we can currently only defend on
fast CPUs.
...
written down.** Our fictional VHDX-expansion story was plausible, internally
...
`wire_bytes` — the counter we reached for to measure a metadata-compression
